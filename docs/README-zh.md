...
1. 配置谷歌学术引用爬虫：
    ...
    1. 在 GitHub 本仓库页面的 `Action` 中，点击 *"I understand my workflows, go ahead and enable them"* 启用 workflow。该 Action 每天 08:00 UTC 运行一次，将 Google Scholar 引用统计数据发布到本仓库的 `google-scholar-stats` 分支，也可以手动触发。若 Google Scholar 暂时不可用，workflow 会保留上一版引用数据。
